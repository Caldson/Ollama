...
class EasyOllamaBeta:
    def __init__(self, api_key, model='qwen3.5:0.8b', base_url='http://localhost:11434/', chinese_mode=True): # Use qwen3.5:0.8b for best experience.
        self.model = model
    ...
    def read_file(self, input):
        ...
        try:
            ...
            self.chat(f'文件不存在：{file_path}', role='system')
    ...
    def edit_file(self, input):
        ...
        try:
            if os.path.exists(file_path):
                ...
                self.chat(f'成功创建并编辑文件：{file_path}', role='system')
        except Exception as e:
            self.aout(f"[编辑文件失败：{file_path}]" if self.chinese_mode else f"[Edit failed {file_path}]")
            self.chat(f'编辑文件失败：{file_path}，错误信息：{e}', role='system')
    def get_time(self, input):
        ...
        try:
            if format == '1':
                ...
                self.chat(f'时间格式错误：{format}，请输入1、2、3中的一个。', role='system')
        except Exception as e:
            self.aout("[时间获取失败]" if self.chinese_mode else "[Time get failed]")
            self.chat(f'获取时间失败：{format}，错误信息：{e}', role='system')
